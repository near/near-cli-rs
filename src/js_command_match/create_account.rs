...
pub struct CreateAccountArgs {
    ...
    #[clap(long, aliases = ["initialBalance"], default_missing_value = Some("10"))]
...
impl CreateAccountArgs {
    pub fn to_cli_args(&self, network_config: String) -> Vec<String> {
        ...
        if self.use_faucet {
            ...
            command.push(self.account_id.to_owned());
            command.push(format!("\"{} NEAR\"", self.initial_balance.clone().unwrap()));
        }
        ...
        if self.public_key.is_some() {
            ...
            command.push(
                self.public_key
                    .to_owned()
                    .expect("Valid public key must be provided"),
            );
        };
 
        if !self.public_key.is_some() && !self.use_ledger {
...
mod tests {
    ...
    fn create_account_using_faucet_testnet() {
        ...
        assert_eq!(
            result.join(" "),
            "account create-account sponsor-by-faucet-service bob.testnet autogenerate-new-keypair save-to-legacy-keychain network-config testnet send".to_string()
        )
    }
    ...
    fn create_account_using_master_account_testnet() {
        ...
        assert_eq!(
            result.join(" "),
            "account create-account fund-myself bob.testnet \"100 NEAR\" autogenerate-new-keypair save-to-legacy-keychain sign-as alice.testnet network-config testnet send".to_string()
        )
    }
    ...
    fn create_account_using_master_account_with_init_balance_testnet() {
        ...
        assert_eq!(
            result.join(" "),
            "account create-account fund-myself bob.testnet \"1 NEAR\" autogenerate-new-keypair save-to-legacy-keychain sign-as alice.testnet network-config testnet send".to_string()
        )
    }
}