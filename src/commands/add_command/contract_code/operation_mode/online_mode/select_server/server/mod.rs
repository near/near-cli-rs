...
pub enum CliSendFrom {
    /// Specify a sender
    Account(crate::commands::add_command::contract_code::sender::CliSender),
}

#[derive(Debug)]
pub enum SendFrom {
    Account(crate::commands::add_command::contract_code::sender::Sender),
}

impl From<CliSendFrom> for SendFrom {
    fn from(item: CliSendFrom) -> Self {
        match item {
            CliSendFrom::Account(cli_sender) => Self::Account(cli_sender.into()),
        }
    }
}
