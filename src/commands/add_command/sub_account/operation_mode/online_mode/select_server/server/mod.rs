...
pub enum CliSendFrom {
    /// Specify an owner-account
    OwnerAccount(crate::commands::add_command::sub_account::sender::CliSender),
}

#[derive(Debug)]
pub enum SendFrom {
    OwnerAccount(crate::commands::add_command::sub_account::sender::Sender),
}

impl From<CliSendFrom> for SendFrom {
    fn from(item: CliSendFrom) -> Self {
        match item {
            CliSendFrom::OwnerAccount(cli_sender) => {
                Self::OwnerAccount(cli_sender.into())
            }
        }
    }
}
