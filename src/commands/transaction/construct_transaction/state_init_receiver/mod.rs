...
impl StateInitFromJsonContext {
    pub fn from_previous_context(
        ...
        let state_init: near_kit::StateInit = serde_json::from_str(&scope.state_init_json)
...
impl StateInitFromJsonFileContext {
    pub fn from_previous_context(
        ...
        let state_init: near_kit::StateInit = serde_json::from_str(&json_str)
            .wrap_err("Failed to parse JSON-serialized StateInit")?;
        let receiver_account_id = state_init.derive_account_id();
        Ok(Self(StateInitDataContext {
            global_context: previous_context.global_context,
...
impl StateInitDataContext {
    fn build(
        code: near_kit::GlobalContractId,
        global_context: crate::GlobalContext,
        signer_account_id: near_kit::AccountId,
        data: std::collections::BTreeMap<Vec<u8>, Vec<u8>>,
    ) -> color_eyre::eyre::Result<Self> {
        let state_init = near_kit::StateInit::V1(near_kit::StateInitV1 { code, data });
        let receiver_account_id = state_init.derive_account_id();
        Ok(Self {
            global_context,
            signer_account_id,
