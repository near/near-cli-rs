...
fn print_account_creation_status(
    ...
    match result {
        Ok(response) => {
            ...
            let faucet_response: near_kit::RawTransactionResponse = response.json()?;
            let account_creation_transaction = faucet_response.outcome.ok_or_else(|| {
                ...
                near_kit::FinalExecutionStatus::Failure(tx_execution_error) => {
                    ...
                    return match tx_execution_error {
                        near_kit::TxExecutionError::ActionError(action_error) => {
                            crate::common::convert_action_error_to_cli_result(&action_error)
                        }
                        near_kit::TxExecutionError::InvalidTxError(invalid_tx_error) => {
                            crate::common::convert_invalid_tx_error_to_cli_result(&invalid_tx_error)
                        }
                    };
                }
            }
