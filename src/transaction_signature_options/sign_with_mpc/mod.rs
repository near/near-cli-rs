...
impl MpcDeriveKeyContext {
    pub fn from_previous_context(
        ...
        let access_key_view = block_on(network_config.client().rpc().view_access_key(
            &controllable_account,
            &derived_public_key,
            near_kit::BlockReference::optimistic(),
        ))
                .inspect_err(|err| {
                    if matches!(err, near_kit::RpcError::AccessKeyNotFound { .. }) {
                        tracing::error!(
                ...
                move |signed_transaction_to_replace, network_config| {
                    ...
                    let signed_transaction = match sign_outcome_view.status {
                        near_kit::FinalExecutionStatus::SuccessValue(result_b64) => {
                            ...
                            near_kit::SignedTransactionV1 {
                                transaction: near_kit::VersionedTransaction::V0(
