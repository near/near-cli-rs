...
В режиме _offline_ можно создать и подписать транзакцию. Полученный хеш транзакции можно отправить либо позже, либо с другого компьютера. Для входа в режим _offline_ необходимо в команде установить флаг ```--offline```:  
```txt
near --offline tokens \
    fro_volod.testnet \
