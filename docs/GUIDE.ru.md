...
Ответ выводится в формате JSON в stdout, а заголовок `Transaction status:` — в stderr. Для компактного JSON в одну строку используйте `near --quiet transaction view-status ...`. Режим `--teach-me` также выводит трассировку RPC, поэтому для разбора stdout используйте обычный или тихий режим.
